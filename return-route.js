(()=>{
  ...
  function applyDirection(){if(applying)return;applying=true;try{const rows=[...body.querySelectorAll('tr')];if(!rows.length)return;rows.forEach(remember);const ordered=rows.slice().sort((a,b)=>(+a.dataset.routeOrder||0)-(+b.dataset.routeOrder||0));if(direction==='return')ordered.reverse();const start=add15(forwardCourseTime||forwardTimeSelect.value);ordered.forEach((r,i)=>{r.dataset.coordinate=direction==='return'?(r.dataset.returnCoordinate||r.dataset.forwardCoordinate||''):(r.dataset.forwardCoordinate||'');setTime(r,direction==='return'?(i===0?start:''):(r.dataset.forwardTime||''));body.append(r)});forwardTimeSelect.hidden=direction==='return';returnStartLabel.hidden=direction!=='return';returnStartLabel.textContent=direction==='return'?`START ${start}`:'';body.dataset.direction=direction;body.dataset.returnStart=direction==='return'?start:'';body.dispatchEvent(new CustomEvent('route-direction-change',{bubbles:true,detail:{direction,returnStart:start}}))}finally{applying=false}}
  returnSwitch.addEventListener('change',()=>{direction=returnSwitch.checked?'return':'forward';if(direction==='return')forwardCourseTime=forwardTimeSelect.value;applyDirection()});
