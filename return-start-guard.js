(()=>{
  ...
  function reset(){armed=false;warned=false;minStartDistance=Infinity;departureFixes=0;lastPos=null;derivedHeading=null}
  function showWarning(plan){
    let el=document.getElementById('earlyDepartureWarning');
    if(!el){
      el=document.createElement('div');el.id='earlyDepartureWarning';
      el.style.cssText='position:fixed;z-index:99999;left:50%;top:18px;transform:translateX(-50%);width:min(92vw,520px);padding:16px 18px;border:3px solid #fff;border-radius:12px;background:#e11d2e;color:#fff;box-shadow:0 8px 28px #000c;text-align:center;font-size:1.12rem;font-weight:1000;line-height:1.2;letter-spacing:.02em;pointer-events:none';
      document.body.append(el);
    }
    el.innerHTML=`ODJECHAŁEŚ PRZED CZASEM<small style="display:block;margin-top:5px;font-size:.78rem;font-weight:800">Planowany start: ${plan}</small>`;
    el.hidden=false;
    clearTimeout(warningTimer);
    warningTimer=setTimeout(()=>{el.hidden=true},WARNING_MS);
  }
