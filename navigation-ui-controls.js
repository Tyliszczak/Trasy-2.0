(()=>{
  ...
  function init(){
    ...
    if(root.dataset.compactNavUi==='4')return true;
    root.dataset.compactNavUi='4';
    ...
    center.style.cssText='position:fixed;right:12px;top:112px;z-index:50100;width:38px;height:38px;min-width:38px;min-height:38px;padding:0;border:1px solid #fff8;border-radius:19px;background:#111d;color:#fff;font-size:18px;font-weight:900;line-height:38px;box-shadow:0 2px 9px #000a;display:flex;align-items:center;justify-content:center';
    ...
    if(speech&&typeof speech.speak==='function'&&!speech.__routeMuteWrapped){speech.__routeMuteWrapped=true;const nativeSpeak=speech.speak.bind(speech);speech.speak=function(utterance){if(window.__routeVoiceMuted)return;return nativeSpeak(utterance)}}
    function updateVoice(){const muted=window.__routeVoiceMuted===true;voice.textContent=muted?'🔇':'🔊';voice.setAttribute('aria-label',muted?'Włącz komunikaty głosowe':'Wycisz komunikaty głosowe');voice.title=muted?'Włącz komunikaty głosowe':'Wycisz komunikaty głosowe';voice.style.opacity=muted?'.72':'1'}
    voice.onclick=()=>{window.__routeVoiceMuted=!window.__routeVoiceMuted;if(window.__routeVoiceMuted){try{speech?.cancel?.()}catch{}}updateVoice()};updateVoice();
    ...
    const navIcon='<span style="display:block;font-size:26px;line-height:26px;font-weight:900">↑</span>';
    function clearResume(){if(resumeTimer){clearTimeout(resumeTimer);resumeTimer=0}}
    function setOverview(on){
      overview=!!on;
      if(overview){center.innerHTML=navIcon;center.setAttribute('aria-label','Wróć do prowadzenia');center.title='Wróć do prowadzenia'}
      else{center.innerHTML=compassIcon;center.setAttribute('aria-label','Widok na północ');center.title='Widok na północ'}
    }
    setOverview(false);
    function resumeNavigation(){clearResume();if(!overview)return;window.__routeManualView=false;setOverview(false);if(typeof originalCenter==='function')originalCenter.call(center)}
    function scheduleResume(){clearResume();resumeTimer=setTimeout(resumeNavigation,AUTO_RESUME_MS)}
    function enterNorthView(){const map=window.__routeMap;if(!map)return;window.__routeManualView=true;setOverview(true);scheduleResume();try{map.easeTo({bearing:0,pitch:0,zoom:Math.min(map.getZoom(),14.8),duration:500,essential:true})}catch{}}
    ...
    const mapTimer=setInterval(()=>{const map=window.__routeMap;if(!map||map.__compactUiGestures)return;map.__compactUiGestures=true;map.on('dragstart',markManual);map.on('zoomstart',markManual);map.on('rotatestart',markManual);map.on('pitchstart',markManual);clearInterval(mapTimer)},250);
