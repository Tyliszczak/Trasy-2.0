(()=>{
  ...
  function init(){
    ...
    if(!root||!close||!center||!maneuver)return false;if(root.dataset.compactNavUi==='7')return true;root.dataset.compactNavUi='7';
    ...
    function renderState(){
      if(northView){center.innerHTML=navIcon;center.setAttribute('aria-label','Wróć do nawigacji');center.title='Wróć do nawigacji'}
    ...
    function enterNorthView(){const map=window.__routeMap;if(!map)return;northView=true;window.__routeManualView=true;renderState();scheduleResume();try{map.easeTo({bearing:0,pitch:0,zoom:Math.min(map.getZoom(),14.8),duration:500,essential:true})}catch{}}
    renderState();
    center.onclick=()=>{if(northView)resumeNavigation();else enterNorthView()};
    const markManual=()=>{northView=true;window.__routeManualView=true;renderState();scheduleResume()};
    const mapTimer=setInterval(()=>{const map=window.__routeMap;if(!map||map.__compactUiGestures)return;map.__compactUiGestures=true;map.on('dragstart',markManual);map.on('zoomstart',markManual);map.on('rotatestart',markManual);map.on('pitchstart',markManual);clearInterval(mapTimer)},250);setTimeout(()=>clearInterval(mapTimer),30000);return true;
  }
