(()=>{
  ...
  function statusText(detail){const raw=detail?.diffSeconds;if(raw===null||raw===undefined||raw==='')return{kind:'',text:''};const diff=Number(raw);if(!Number.isFinite(diff))return{kind:'',text:''};if(Math.abs(diff)<=30)return{kind:'onTime',text:'👍'};const min=Math.max(1,Math.floor(Math.abs(diff)/60));return diff<0?{kind:'early',text:`${min} min za wcześnie`}:{kind:'late',text:`${min} min opóźnienia`}}
  function render(detail=lastStatusDetail){const el=document.getElementById('routeNextStop');if(!el)return;const d=dataFromRow(activeRow());if(!d){el.textContent='';return}const s=statusText(detail);el.innerHTML=`<span class="nextStopLabel">Następny przystanek</span><span class="nextStopMain">${escapeHtml(d.name)}${d.plan?` · ${escapeHtml(d.plan)}`:''}</span>${s.text?`<span class="nextStopStatus ${s.kind}">${s.text}</span>`:''}`}
  function escapeHtml(v){return String(v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
  body.addEventListener('nav-eta-update',e=>{lastStatusDetail=e.detail;render()});
  body.addEventListener('gps-next-stop-change',()=>{lastStatusDetail=null;render()});
