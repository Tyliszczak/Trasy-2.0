...
async function syncRoutes() {
  if (syncing || !navigator.onLine) { offlineMode = true; updateConnectionStatus(); return; }
  syncing = true;
  try {
...
function normalizeRoute(value, fallbackName = "") {
  ...
  if (!times.length) for (const stop of stops) Object.keys(stop.times).forEach((time) => { if (time && !times.includes(time)) times.push(time); });
...
function normalizeStop(stop, routeTimes) {
  ...
  const times = {};
  if (Array.isArray(sourceTimes)) routeTimes.forEach((time, index) => { times[time] = sourceTimes[index] ?? null; });
  else if (sourceTimes && typeof sourceTimes === "object") Object.entries(sourceTimes).forEach(([time, value]) => { times[String(time)] = value ?? null; });
  if (!name) return null;
...
function isValidRoutes(value) { return Array.isArray(value) && value.length > 0 && value.every((route) => route?.name && Array.isArray(route.times) && Array.isArray(route.stops) && route.stops.length > 0); }
