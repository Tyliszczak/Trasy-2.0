...
const profile=platform?.profile||{mode:'production',requirePlatform:true};
const DATA_KEY=platform?.storageKey('trasy2.routes.v3')||'trasy2.routes.v3.unassigned';
const SYNC_KEY=platform?.storageKey('trasy2.lastSuccessfulSync.v3')||'trasy2.lastSuccessfulSync.v3.unassigned';
const FAIL_KEY=platform?.storageKey('trasy2.firstFailedSync.v3')||'trasy2.firstFailedSync.v3.unassigned';
...
async function startApp(){
  ...
  else{routes=[];renderRoutes();routeSelect.disabled=true;message.textContent='Sprawdzanie dostępu do danych firmy…'}
  const ok=await syncRoutes();
  if(!ok){
    message.textContent=profile.requirePlatform
      ?'Nie można pobrać danych firmy. Uruchom aplikację z aktywnego linku kierowcy albo sprawdź połączenie.'
      :'Używana jest ostatnia bezpieczna kopia danych firmy.';
  }else message.textContent=routes.length?'':'Administrator firmy nie opublikował jeszcze tras.';
  routeSelect.disabled=!routes.length;
}
...
document.addEventListener('trasy:platform-session-expired',event=>{offline=true;updateStatus();message.textContent='Dostęp kierowcy wygasł lub został zablokowany. Otwórz ponownie aktualny link z panelu firmy.';routeSelect.disabled=true;document.documentElement.dataset.platformError=String(event.detail?.code||'session-expired')});
