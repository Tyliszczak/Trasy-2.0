(()=>{
  ...
  async function openMapNav(startIndex){
    ...
    try{
      ...
      if(!map){map=new maplibregl.Map({container:'routeMapCanvas',style:{version:8,sources:{osm:{type:'raster',tiles:['https://tile.openstreetmap.org/{z}/{x}/{y}.png'],tileSize:256,attribution:'© OpenStreetMap contributors'}},layers:[{id:'osm',type:'raster',source:'osm'}]},center:[origin[1],origin[0]],zoom:ZOOM,pitch:PITCH,bearing:0,attributionControl:true,maxPitch:60});map.addControl(new maplibregl.NavigationControl({showCompass:true,showZoom:true}),'bottom-right');map.on('dragstart',e=>{if(e.originalEvent)autoCenter=false});await new Promise(resolve=>map.loaded()?resolve():map.once('load',resolve))}else{map.resize();if(map.getSource('route'))map.getSource('route').setData(routeGeoJSON([]))}
      clearMarkers();
