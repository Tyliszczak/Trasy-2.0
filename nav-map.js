(()=>{
  ...
  function speak(text,d){let bucket=d<70?'now':d<220?'200':d<550?'500':'';if(!bucket)return;const key=text+'|'+bucket;if(key===lastSpoken)return;lastSpoken=key;try{speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(bucket==='now'?text:`Za ${bucket==='200'?'200':'500'} metrów. ${text}`);u.lang='pl-PL';speechSynthesis.speak(u)}catch{}}
  function updateGuidance(ll){const {step,distance}=nearestStep(ll);if(!step)return;const text=instruction(step);maneuverEl.textContent=text;maneuverDistance.textContent=fmtDistance(distance);speak(text,distance);if(currentStops.length){let best=currentStops[0],d=hav(ll,best.coord);for(const s of currentStops){const x=hav(ll,s.coord);if(x<d){best=s;d=x}}nextStopEl.textContent=`Najbliższy punkt: ${best.name}`}const off=nearestRouteDistance(ll);status.textContent=off>80?'Poza trasą — przeliczam…':`Na trasie • dokładność GPS ${Math.round(window.__navAcc||0)} m`;if(off>80&&!rerouteTimer)rerouteTimer=setTimeout(()=>{rerouteTimer=0;buildRoute(ll,currentStops)},1800)}
  ...
  async function openMapNav(startIndex){
    if(!window.maplibregl){alert('Mapa nie została załadowana. Sprawdź połączenie z internetem.');return}
    ...
    try{
      ...
      if(!map){map=new maplibregl.Map({container:'routeMapCanvas',style:{version:8,sources:{osm:{type:'raster',tiles:['https://tile.openstreetmap.org/{z}/{x}/{y}.png'],tileSize:256,attribution:'© OpenStreetMap contributors'}},layers:[{id:'osm',type:'raster',source:'osm'}]},center:[origin[1],origin[0]],zoom:ZOOM,pitch:PITCH,bearing:0,attributionControl:true,maxPitch:60});map.addControl(new maplibregl.NavigationControl({showCompass:true,showZoom:true}),'bottom-right');map.on('dragstart',()=>{autoCenter=false});map.on('rotatestart',()=>{if(map.isMoving())autoCenter=false});map.on('pitchstart',()=>{if(map.isMoving())autoCenter=false});await new Promise(resolve=>map.loaded()?resolve():map.once('load',resolve))}else{map.resize();if(map.getSource('route'))map.getSource('route').setData(routeGeoJSON([]))}
      clearMarkers();
      positionMarker=new maplibregl.Marker({element:markerElement('▲','#ccff33',34),rotationAlignment:'viewport',pitchAlignment:'viewport',subpixelPositioning:true}).setLngLat([origin[1],origin[0]]).addTo(map);
      stops.forEach((s,i)=>{const marker=new maplibregl.Marker({element:markerElement(String(i+1),i===stops.length-1?'#ffb000':'#078df0')}).setLngLat([s.coord[1],s.coord[0]]).setPopup(new maplibregl.Popup({offset:18}).setText(s.name)).addTo(map);stopMarkers.push(marker)});
      await buildRoute(origin,stops);updateGuidance(origin);followCamera(origin,0,true);
      if(watchId!==null)navigator.geolocation.clearWatch(watchId);
      watchId=navigator.geolocation.watchPosition(p=>{const ll=[p.coords.latitude,p.coords.longitude];window.__navAcc=p.coords.accuracy||0;if(positionMarker)positionMarker.setLngLat([ll[1],ll[0]]);const h=headingFromPosition(p,ll);if(positionMarker)positionMarker.setRotation(0);followCamera(ll,h,false);updateGuidance(ll)},()=>{status.textContent='Brak aktualnej pozycji GPS.'},{enableHighAccuracy:true,maximumAge:1000,timeout:15000});
