(()=>{
  ...
  function directionsUrl(coords,start){
    const c=coords.slice(start).filter(Boolean);if(!c.length)return null;
    const u=new URL('https://www.google.com/maps/dir/');
    u.searchParams.set('api','1');
    if(c.length===1){u.searchParams.set('destination',c[0])}
    else{
      u.searchParams.set('origin',c[0]);
      u.searchParams.set('destination',c.at(-1));
      if(c.length>2)u.searchParams.set('waypoints',c.slice(1,-1).join('|'));
    }
